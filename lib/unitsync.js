...
setTimeout(function(){ console.log('usync called');libunitsync.Init(0,0);loading(false);}, 1000);

	
	
}

setTimeout(usyncLoad, 10000); 

function listAllLocalMapNames() {
    isOpen = libunitsync.Init(0, 0);   //this will hang the process!
...
function showMinimap(mapName, reduction=0) {
    ...
    buffer = libunitsync.GetMinimap(mapName, reduction).reinterpret(width * height * 2);
